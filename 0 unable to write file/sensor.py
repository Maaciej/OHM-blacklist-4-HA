...
from homeassistant.const import CONF_HOST, CONF_PORT, CONF_GATHER_INPUT, CONF_BLACKLIST
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {  vol.Required(CONF_HOST): cv.string
     ...
     , vol.Optional(CONF_GATHER_INPUT, default=False): bool 
     , vol.Optional(CONF_BLACKLIST, default=False): bool}
)
...
#        input_list_content = []
#        blacklist_content = []
        #do not know how to find configuration folder automatically for different instalation methods
        input_list = "../../../tmp/OHM_" + str( self._config.get( CONF_HOST ) ) + "_input_list.txt"
        _LOGGER.debug ("gathering input filename = " + input_list + "pwd " + str (os.getcwd()) )

        self.devices = self.parse_children(self.data, [], [], [])

#        if self._config.get( CONF_GATHER_INPUT ): # and len(input_list_content):
        _LOGGER.debug("saving list "+str(len(input_list_content)) )

        with open("costam2", 'x', encoding ="utf8") as output:
#            for item in sorted( set( input_list_content ) ):
                _LOGGER.debug("saving " + item )
#                output.write( f"{item}\n")
                ouput.write("byleco")
    ...
    def parse_children(self, json, devices, path, names):
        ...
        if self._config.get( CONF_GATHER_INPUT ):
            if fullname in blacklist_content:
                input_list_content.append(fullname + "*BLACKLISTED*")
            else:
